...
use od_core::{Account, AccountId, Credentials, Registrar, RegistrationState, SignalingPort, Transport};
use od_sip::SipAgent;
...
fn rejects_invalid_credentials_as_permanent() {
    if !testbed_is_reachable() {
        panic!("banc de test injoignable — lancez `docker compose -f testbed/docker-compose.yml up -d`");
    }
...
fn second_account_registers_independently() {
    ...
    let first = agent
        .register(&account("1001", "opendial"))
        .expect("enregistrement du premier compte");
    let second = agent
        .register(&account("1002", "opendial"))
